...
public class Buffer {
    ...
    public void encrypt(BigInteger exponent, BigInteger modulus) {
        ...
        offset = 0;
        System.out.println("abyte1="+abyte1.length);
        putByte(abyte1.length);
        putBytes(abyte1, 0, abyte1.length);
    }

    public void decrypt(BigInteger d, BigInteger n, int l) {
      offset = 0;
      byte[] buf = new byte[l];
      getBytes(buf, 0, l);
      System.out.println("buf="+Arrays.toString(buf));
      BigInteger bi2 = new BigInteger(buf);
      BigInteger bi3 = bi2.modPow(d, n);
      byte decrypted[] = bi3.toByteArray();
      offset = 0;
      putBytes(decrypted, 0, decrypted.length);
    }
