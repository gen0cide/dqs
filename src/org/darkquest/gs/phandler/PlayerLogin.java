...
public class PlayerLogin implements PacketHandler {
  ...
  public void handlePacket(org.darkquest.gs.connection.Packet p, IoSession session) throws Exception {
    ...
    if(loginCode == 0 || loginCode == 1 || loginCode == 99) {
      ...
      /* Start of XP point system */

      if(!player.getCache().hasKey("xppoints")) {
        player.getCache().store("xppoints", 0L);
      }
      sender.sendCacheableLong("xppoints", player.getCache().getLong("xppoints"));


      /* End of XP point system */
